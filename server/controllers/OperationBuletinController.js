...
exports.generateDataSheet = async (req, res) => {
  ...
  try {
    ...
    sheet.getCell("D2").value = style.customer?.customer_name || "";

    sheet.mergeCells("B3:C3");
    sheet.getCell("B3").value = "Style No";
    sheet.getCell("B3").font = { bold: true };
    sheet.mergeCells("D3:E3");
    sheet.getCell("D3").value = style.style_no || "";

    sheet.mergeCells("B4:C4");
    sheet.getCell("B4").value = "Date";
    sheet.getCell("B4").font = { bold: true };
    sheet.mergeCells("D4:E4");
    sheet.getCell("D4").value = new Date().toLocaleDateString();

    for (let r = 2; r <= 4; r++) {
      applyRowBorder(sheet.getRow(r));
    }
    ...
    headers.forEach((h, i) => {
      applyHeader(sheet.getCell(6, i + 1), h);
    });
    ...
    for (const op of style.operations) {
      const mainRow = sheet.getRow(rowIndex);
      mainRow.getCell(1).value = opNo++;
      mainRow.getCell(2).value = op.operation_name;

      for (let c = 1; c <= TOTAL_COLS; c++) {
        const cell = mainRow.getCell(c);
        cell.font = { bold: true };
        cell.fill = {
          type: "pattern",
          pattern: "solid",
      ...
      applyRowBorder(mainRow);
      rowIndex++;

      for (const sub of op.subOperations) {
        const r = sheet.getRow(rowIndex);

        r.getCell(1).value = sub.sub_operation_number || "";
        r.getCell(2).value = "  " + sub.sub_operation_name;
        r.getCell(3).value =
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=technical-${style.style_no}.xlsx`,
    );
