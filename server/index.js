...
app.get("/videos/:videoUrl(*)", (req, res) => {
  const videoParam = decodeURIComponent(req.params.videoUrl);

  // Security
  if (!videoParam || videoParam.includes("..")) return res.status(403).end();

  const filename = path.basename(videoParam);
...
app.get("/helper-videos/:videoUrl", (req, res) => {
...
app.get("/helper-images/:filename(*)", (req, res) => {
  const filename = req.params.filename;
  ...
  // Build the full path - images are in SubOpImages folder
  const imagePath = path.join(STORAGE_UNC_PATH, "HelperOpImages", filename);
...
app.get("/style-images/:filename(*)", (req, res) => {
  const filename = req.params.filename;
